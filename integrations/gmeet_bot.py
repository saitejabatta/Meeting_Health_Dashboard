...
class GoogleMeetBot:
    ...
    def join(self, meet_url: str) -> JoinResult:
        ...
        try:
            ...
            return JoinResult(False, self.manual_join_instructions(meet_url, "Selenium or undetected-chromedriver is not installed."))
        try:
            ...
            join_button.click()
            return JoinResult(True, f"Opened Google Meet and attempted to join. Route audio through {routing.device_name}.")
        except Exception as exc:
            return JoinResult(False, self.manual_join_instructions(meet_url, str(exc)))
    ...
    def manual_join_instructions(self, meet_url: str, reason: str = "") -> str:
        """Return manual Google Meet joining instructions."""
        reason_text = f" Automatic join failed: {reason}" if reason else ""
        return (
            f"Open {meet_url} manually and join the meeting.{reason_text} "
